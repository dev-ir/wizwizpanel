...
<div class="flex flex-col flex-1 w-full">
    ...
    <main class="h-full pb-16 overflow-y-auto">
        <div class="container grid px-6 mx-auto ">
            ...
            >
                <svg fill="gray" xmlns="http://www.w3.org/2000/svg" id="Outline" viewBox="0 0 24 24" width="21" height="21"><path d="M18.4,7.379a1.128,1.128,0,0,1-.769-.754h0a8,8,0,1,0-15.1,5.237A1.046,1.046,0,0,1,2.223,13.1,5.5,5.5,0,0,0,.057,18.3,5.622,5.622,0,0,0,5.683,23H11a1,1,0,0,0,1-1h0a1,1,0,0,0-1-1H5.683a3.614,3.614,0,0,1-3.646-2.981,3.456,3.456,0,0,1,1.376-3.313A3.021,3.021,0,0,0,4.4,11.141a6.113,6.113,0,0,1-.073-4.126A5.956,5.956,0,0,1,9.215,3.05,6.109,6.109,0,0,1,9.987,3a5.984,5.984,0,0,1,5.756,4.28,2.977,2.977,0,0,0,2.01,1.99,5.934,5.934,0,0,1,.778,11.09.976.976,0,0,0-.531.888h0a.988.988,0,0,0,1.388.915c4.134-1.987,6.38-7.214,2.88-12.264A6.935,6.935,0,0,0,18.4,7.379Z"/><path d="M18.707,16.707a1,1,0,0,0,0-1.414l-1.586-1.586a3,3,0,0,0-4.242,0l-1.586,1.586a1,1,0,0,0,1.414,1.414L14,15.414V23a1,1,0,0,0,2,0V15.414l1.293,1.293a1,1,0,0,0,1.414,0Z"/></svg>
            ...
            <div style='margin-top:40px' class=" w-full flex justify-center items-center grid gap-6 mb-8 xl:grid-cols-6 lg:grid-cols-3 md:grid-cols-2 sm:grid-cols-2 grid-cols-2">
                ...
                $sql_servers = "SELECT * FROM servers ORDER BY id DESC";
				$stmt = $conn->prepare($sql_servers);
				$stmt->execute();
				$result_servers = $stmt->get_result();
				$stmt->close();
...
<div class="w-full p-4 m-auto bg-white shadow-sm rounded-2xl dark:bg-gray-800">
    <div class="w-full h-full text-center">
        <div class="flex flex-col justify-between h-full">
            <img src="icons/servers.svg" width="40px" class="mx-auto mt-4" style="color: #5f7d95">
...
>
    ...
    >
        ...
        if (isset($_GET['delete'])) {
            $id_delete_server = $_GET['delete'];
			if(is_numeric($id_delete_server)){
				$sql_delete_server = "DELETE FROM servers WHERE id=?";
				$stmt = $conn->prepare($sql_delete_server);
				$stmt->bind_param('i', $id_delete_server);
				$stmt->execute();
        ...
        if (isset($_GET['on'])) {
			...
			if(is_numeric($id_on_select)){
				$sql_on_select = "UPDATE servers SET status='1' WHERE id=?";
        ...
        if (isset($_GET['off'])) {
			...
			if(is_numeric($id_off_select)){
				$sql_off_select = "UPDATE servers SET status='0' WHERE id='$id_off_select'";
        ...
        if(isset($_POST['action']) && $_POST['action'] == 'insert_server_backup') {
            $ip_server = $_POST['ip'];
            $port_server = $_POST['port'];
            $username_server = $_POST['username'];
            $password_server = $_POST['password'];
            $name_server = $_POST['name'];
            $panel_server = $_POST['panel'];

            if($panel_server == "NidukaAkalanka") $panel_server = "/etc/x-ui-english/x-ui-english.db";
            else $panel_server = "/etc/x-ui/x-ui.db";
            $status = '1';
            $insert_server_sql = "INSERT INTO servers (ip,port,username,password,name,status,panel) 
                          VALUES (?,?,?,?,?,?,?)";
			$stmt = $conn->prepare($insert_server_sql);
			$stmt->bind_param('sssssss',$ip_server,$port_server,$username_server,$password_server,$name_server,$status,$panel_server);
			$stmt->execute();
        ...
        ?>
        <form id="insert_server_backup">
            <!-- Modal body -->
                <p class="m-4 dark:text-gray-200 text-gray-700">
                    <?php echo $_LANG['notbackup']?>
                </p>
                <div class="flex relative m-2">
                    <span class="dark:border-gray-600 dark:bg-gray-700 focus:border-purple-400 focus:outline-none focus:shadow-outline-purple dark:text-gray-300 dark:focus:shadow-outline-gray rounded-l-md inline-flex  items-center px-3 border-t bg-white border-l border-b  border-gray-300 text-gray-500 shadow-sm text-sm">
                        <?php echo $_LANG['servername']?>
                    </span>
                    <input name="name" style="font-size: 14px;" value="" type="text" id="with-email"
                ...
                <div class="flex relative m-2">
                    ...
                    <input name="ip" style="font-size: 14px;" value="" type="text" id="with-email"
                           class="dark:border-gray-600 dark:bg-gray-700 focus:border-purple-400 focus:outline-none focus:shadow-outline-purple dark:text-gray-300 dark:focus:shadow-outline-gray rounded-r-lg flex-1 appearance-none border border-gray-300 w-full py-2 px-4 bg-white text-red-400 placeholder-gray-400 shadow-sm text-base focus:outline-none focus:ring-2 focus:ring-blue-200 focus:border-transparent"
                           placeholder="" required/>
                </div>
                <div class="flex relative m-2">
                    ...
                    <input name="port" style="font-size: 14px;" value="" type="number" id="with-email"
                           class="dark:border-gray-600 dark:bg-gray-700 focus:border-purple-400 focus:outline-none focus:shadow-outline-purple dark:text-gray-300 dark:focus:shadow-outline-gray rounded-r-lg flex-1 appearance-none border border-gray-300 w-full py-2 px-4 bg-white text-red-400 placeholder-gray-400 shadow-sm text-base focus:outline-none focus:ring-2 focus:ring-blue-200 focus:border-transparent"
                           placeholder="" required/>
                </div>
                <div class="flex relative m-2">
                    ...
                    <input name="username" style="font-size: 14px;" value="" type="text" id="with-email"
                           class="dark:border-gray-600 dark:bg-gray-700 focus:border-purple-400 focus:outline-none focus:shadow-outline-purple dark:text-gray-300 dark:focus:shadow-outline-gray rounded-r-lg flex-1 appearance-none border border-gray-300 w-full py-2 px-4 bg-white text-red-400 placeholder-gray-400 shadow-sm text-base focus:outline-none focus:ring-2 focus:ring-blue-200 focus:border-transparent"
                           placeholder="" required/>
                </div>
                <div class="flex relative m-2">
                    ...
                    <input name="password" style="font-size: 14px;" value="" type="text" id="with-email"
                           class="dark:border-gray-600 dark:bg-gray-700 focus:border-purple-400 focus:outline-none focus:shadow-outline-purple dark:text-gray-300 dark:focus:shadow-outline-gray rounded-r-lg flex-1 appearance-none border border-gray-300 w-full py-2 px-4 bg-white text-red-400 placeholder-gray-400 shadow-sm text-base focus:outline-none focus:ring-2 focus:ring-blue-200 focus:border-transparent"
                           placeholder="" required/>
                </div>
            <label class="block mt-2 text-sm mx-auto" style="width: 97%;line-height: 16px ">
                <select required name="panel"
                        class="block w-full mt-1 text-sm dark:text-gray-300 dark:border-gray-600 dark:bg-gray-700 form-select focus:border-purple-400 focus:outline-none focus:shadow-outline-purple dark:focus:shadow-outline-gray">
                    <option value="Vaxilu">Vaxilu</option>
                    <option value="Alireza">Alireza</option>
                    <option value="Mhsanaei">Mhsanaei</option>
                    <option value="FranzKafkaYu">FranzKafkaYu</option>
                    <option value="Hossinasaadi">Hossinasaadi</option>
                    <option value="HexaSoftwareTech">HexaSoftwareTech</option>
                    <option value="NidukaAkalanka">NidukaAkalanka</option>
                </select>
            </label>
                <footer class="mt-5 flex flex-col items-center justify-end px-6 py-3 -mx-6 -mb-4 space-y-4 sm:space-y-0 sm:space-x-6 sm:flex-row bg-gray-50 dark:bg-gray-800">
                    <button type="button" style="border: 1px solid #e76d81"
                        ...
                        <svg xmlns="http://www.w3.org/2000/svg" fill="#e76d81" id="Outline" viewBox="0 0 24 24" width="15">
                    ...
                    <button style="border: 1px solid #41c76e" name="add_server" type="submit" @click="closeModal"
                        ...
                        <svg xmlns="http://www.w3.org/2000/svg" id="Outline" fill="#41c76e" viewBox="0 0 24 24" width="15">
